...
int demo_init( demo_t *pdemo )
{
	...
	pdemo->ppzfmc2_iic = &(pdemo->pzfmc2_iic);
   ...
   xil_printf( "PicoZed FMC Carrier2 I2C Initialization ...\n\r" );
   status = fmc_iic_xps_init(pdemo->ppzfmc2_iic,"PicoZed FMC Carrier2 I2C Controller", XPAR_AXI_IIC_0_BASEADDR );
   if ( !status )
   {
	  xil_printf( "ERROR : Failed to open PZFMC2-IIC driver\n\r" );
	  exit(0);
   }

   {
	   ...
	   xil_printf( "\tI2C Scan ...\n\r" );
	   for ( i2c_dev = 0; i2c_dev < 128; i2c_dev++ )
	   {
          i2c_addr = 0x00;
          i2c_data = 0x00;
		  num_bytes = pdemo->ppzfmc2_iic->fpIicRead( pdemo->ppzfmc2_iic, i2c_dev, i2c_addr, &i2c_data, 1);
		  if (num_bytes < 1)
		  {
		  ...
		  {
			 xil_printf("\tDetected I2C device at address 0x%02X[0x%02X] => 0x%02X\n\r", (i2c_dev << 1), i2c_addr, i2c_data);
		  }
	   }
   }
...
int demo_start_hdmi_out( demo_t *pdemo )
{
	int status;
...
#if 1
    //pDemo->fmchc_hdmio_timing.IsHDMI        = 1; // HDMI Mode
    pdemo->fmchc_hdmio_timing.IsHDMI        = 0; // DVI Mode
    pdemo->fmchc_hdmio_timing.IsEncrypted   = 0;
    pdemo->fmchc_hdmio_timing.IsInterlaced  = 0;
    pdemo->fmchc_hdmio_timing.ColorDepth    = 8;

    pdemo->fmchc_hdmio_timing.HActiveVideo  = 1920;
    pdemo->fmchc_hdmio_timing.HFrontPorch   =   88;
    pdemo->fmchc_hdmio_timing.HSyncWidth    =   44;
    pdemo->fmchc_hdmio_timing.HSyncPolarity =    1;
    pdemo->fmchc_hdmio_timing.HBackPorch    =  148;
    pdemo->fmchc_hdmio_timing.VBackPorch    =   36;

    pdemo->fmchc_hdmio_timing.VActiveVideo  = 1080;
    pdemo->fmchc_hdmio_timing.VFrontPorch   =    4;
    pdemo->fmchc_hdmio_timing.VSyncWidth    =    5;
    pdemo->fmchc_hdmio_timing.VSyncPolarity =    1;

    xil_printf( "FMC HDMI Output Initialization ...\n\r" );
    status = fmc_hdmi_cam_hdmio_init( pdemo->pfmc_hdmi_cam,
  	                             1,                      // hdmioEnable = 1
  	                             &(pdemo->fmchc_hdmio_timing), // pTiming
  	                             0                       // waitHPD = 0
  	                             );
    if ( !status )
    {
...
#if 1
    //pDemo->pzfmc2_hdmio_timing.IsHDMI        = 1; // HDMI Mode
    pdemo->pzfmc2_hdmio_timing.IsHDMI        = 0; // DVI Mode
    pdemo->pzfmc2_hdmio_timing.IsEncrypted   = 0;
    pdemo->pzfmc2_hdmio_timing.IsInterlaced  = 0;
    pdemo->pzfmc2_hdmio_timing.ColorDepth    = 8;

    pdemo->pzfmc2_hdmio_timing.HActiveVideo  = 1920;
    pdemo->pzfmc2_hdmio_timing.HFrontPorch   =   88;
    pdemo->pzfmc2_hdmio_timing.HSyncWidth    =   44;
    pdemo->pzfmc2_hdmio_timing.HSyncPolarity =    1;
    pdemo->pzfmc2_hdmio_timing.HBackPorch    =  148;
    pdemo->pzfmc2_hdmio_timing.VBackPorch    =   36;

    pdemo->pzfmc2_hdmio_timing.VActiveVideo  = 1080;
    pdemo->pzfmc2_hdmio_timing.VFrontPorch   =    4;
    pdemo->pzfmc2_hdmio_timing.VSyncWidth    =    5;
    pdemo->pzfmc2_hdmio_timing.VSyncPolarity =    1;

    xil_printf( "PicoZed FMC Carrier2 on-board HDMI Output Initialization ...\n\r" );
    status = pzfmc2_hdmio_init( pdemo->ppzfmc2_iic,
  	                             1,                      // hdmioEnable = 1
  	                             &(pdemo->pzfmc2_hdmio_timing), // pTiming
  	                             0                       // waitHPD = 0
  	                             );
    if ( !status )
    {
