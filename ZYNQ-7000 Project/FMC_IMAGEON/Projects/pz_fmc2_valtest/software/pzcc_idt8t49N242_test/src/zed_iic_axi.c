...
int zed_iic_axi_IicRead ( zed_iic_t *pIIC, uint8_t ChipAddress, uint16_t RegAddress,
                                           uint8_t *pBuffer, uint8_t ByteCount);
                                         ...
                                         uint8_t *pBuffer, uint8_t ByteCount)
{
  ...
  uint8_t ControlReg;
  uint8_t StatusReg;
  uint8_t RegAddr[2];		// to adjust byte order of address
  int cnt = 0;
  zed_iic_axi_t *pContext = (zed_iic_axi_t *)(pIIC->pContext);

  RegAddr[0] = RegAddress>>8;
