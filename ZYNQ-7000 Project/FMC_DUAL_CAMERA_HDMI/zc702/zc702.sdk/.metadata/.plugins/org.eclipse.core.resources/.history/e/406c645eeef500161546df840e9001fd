...
//#if defined(XPAR_CFA_NUM_INSTANCES)
   ...
   XCfa *xcfa;
   XCfa_Config xcfa_config;
   xcfa_config.BaseAddress = pDemo->uBaseAddr_CFA;
   xcfa->Config = xcfa_config;

   xil_printf( "Color Filter Array Interpolation (CFA) Initialization ...\n\r" );
   XCfa_Reset( xcfa );
   XCfa_SyncReset( xcfa );
   XCfa_Enable( xcfa );
   XCfa_RegUpdateDisable( xcfa );
   XCfa_WriteReg( pDemo->uBaseAddr_CFA, CFA_BAYER_PHASE, 0 );
   XCfa_RegUpdateEnable( xcfa );
   xil_printf("\tCFA done\r\n");
#endif
